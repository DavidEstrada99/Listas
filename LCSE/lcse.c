/*!
  ...
  *\brief operaiones basicas de la lista circular simplemente enlazada 
  */
...
int eliminar( Lista* lista, int pos ){
	...
	if( lista!=NULL ){
		if( lista->inicio!=NULL ){
			...
			else if( pos==ultimo||pos==lista->tam|| pos == -1){
				     Nodo* aux_2;
				     aux=lista->inicio;
				     aux_2=lista->inicio->sig;
				     while( aux_2->sig!=lista->inicio ){
				     	    aux_2=aux_2->sig;
				     	    aux=aux->sig;
					 }
